...
她会在合适的时机主动找你说话，不是机械的定时任务。

### 定时问候

- **早安**：每天 7:00-10:00 随机（±15 分钟抖动），不会正好 8:00
- **晚安**：每天 21:00-0:00 随机
...
新用户默认「正常」。50% 静默触发时改为发一条日常分享，而非"你怎么不理我"。

### 随机日常分享

每天 2-3 次，活跃时段（9:00-23:00）随机触发。内容由 LLM 生成，注入六层记忆上下文：

> "刚才看到一只超胖的橘猫，想到你了"  
> "今天高数好难...你那边怎么样"  
> "凌晨 debug 终于找到那个 bug，原来是少了个分号"

不是模板——根据人设和最近记忆每次都不一样。独立计数器，不受定时问候上限限制。
