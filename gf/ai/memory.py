...
如果对话中有**值得永久记住的原话**（深情告白、脆弱倾诉、重要承诺、特别温柔的话），
在 archive 中列出。最多 3 条。
...
```json
{
  ...
  "archive": [
    {
      "content": "用户原话（直接引用，50字以内）",
      "context": "一句话背景（10字内）",
      "emotion": "vulnerable|intimate|happy|sad|anxious|excited|tired|grateful",
      "importance": 8
    }
  ]
}
...
class MemoryManager:
    ...
    async def maybe_summarize(self, user_id: str, recent_messages: list[dict],
        ...
        try:
            raw = await _lite_chat(SUMMARIZE_PROMPT, conversation[:4000], max_tokens=400)
            data = _parse_json(raw)
            archive_entries = data.pop("archive", []) if data.get("summary") else []
            if data.get("summary"):
                data["high_importance"] = data.get("high_importance", False)
                logger.info(f"Summarized for {user_id}: {data.get('key_topics', [])}")
            return data if data.get("summary") else None, archive_entries
        except Exception as e:
    ...
    async def extract_moments(self, user_id: str, recent_messages: list[dict],
                               total_msgs: int) -> tuple[list[dict], list[dict]]:
        """Scan conversation for shared moments. Returns (moments, archive_entries)."""
        conversation = "\n".join(
        ...
        try:
            raw = await _lite_chat(
                "扫描对话，提取共同瞬间+值得永久保存的原话。只输出JSON：{\"moments\":[{\"content\":\"...\",\"importance\":1-10}],\"archive\":[{\"content\":\"原话\",\"context\":\"背景\",\"emotion\":\"vulnerable|intimate|happy|sad|anxious|excited|tired|grateful\",\"importance\":8}]}",
                conversation[:3000], max_tokens=400,
            )
            data = _parse_json(raw)
            ...
            for a in data.get("archive", []):
                if a.get("importance", 5) >= 8:
                    archives.append(a)
        except Exception as e:
            logger.debug(f"Moment extraction skipped: {e}")
        return moments, archives
